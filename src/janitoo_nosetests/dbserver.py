...
def jntt_docker_dbserver(module_name, cls, prefix='Class'):
    ...
    for name, conf in DBCONFS:
        setattr(sys.modules[module_name], 'DBServer%s%s'%(prefix,name), type('DBServer%s%s'%(prefix,name), (JNTTDBDockerServerCommon, cls, JNTTDBDockerServer), {'dbconf': (name, conf)}))
