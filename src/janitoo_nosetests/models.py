...
def jntt_docker_models(module_name, cls, prefix='Class'):
    ...
    for name, conf in DBCONFS:
        setattr(sys.modules[module_name], 'JNTTDockerModels%s%s'%(prefix,name), type('JNTTDockerModels%s%s'%(prefix,name), (JNTTDockerModels,cls), {'dbconf': (name, conf)}))
...
def jntt_docker_fullmodels(module_name, cls, prefix='Class'):
    ...
    """
    for name, conf in DBCONFS:
        setattr(sys.modules[module_name], 'JNTTDockerFullModels%s%s'%(prefix,name), type('JNTTDockerFullModels%s%s'%(prefix,name), (JNTTDockerFullModels,cls), {'dbconf': (name, conf)}))
